import validator, { matchedData, validationResult } from "express-validator"
...
})) // mocking the express validator to test
...
describe('create users',()=>{
    ...
    it('should return status of 201 and the user created',async()=>{
        ...
        expect(User.mock.instances[0].save).toHaveBeenCalled() //same as the below  one
        expect(saveMethod).toHaveBeenCalled()// same as the above one
    ...
    it('should send status of 400 when database fails to save user',async () =>{
        ...
        const saveMethod = jest.spyOn(User.prototype,"save").mockImplementationOnce(
            () => Promise.reject("Failed to save user"))
        await createUserHandler(mockRequest,mockResponse)
        expect(saveMethod).toHaveBeenCalled()
        expect(mockResponse.status).toHaveBeenCalledWith(400)
        expect(mockResponse.send).toHaveBeenCalledWith({ error: 'Failed to save user' })
    })
})